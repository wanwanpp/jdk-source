...
class NativeBuffers {
    ...
    private static ThreadLocal<sun.nio.fs.NativeBuffer[]> threadLocal =
        new ThreadLocal<sun.nio.fs.NativeBuffer[]>();

    /**
     * Allocates a native buffer, of at least the given size, from the heap.
     */
    static sun.nio.fs.NativeBuffer allocNativeBuffer(int size) {
        // Make a new one of at least 2k
        if (size < 2048) size = 2048;
        return new sun.nio.fs.NativeBuffer(size);
    }
     ...
     * local cache.
     */
    static sun.nio.fs.NativeBuffer getNativeBufferFromCache(int size) {
        // return from cache if possible
        sun.nio.fs.NativeBuffer[] buffers = threadLocal.get();
        if (buffers != null) {
            for (int i=0; i<TEMP_BUF_POOL_SIZE; i++) {
                sun.nio.fs.NativeBuffer buffer = buffers[i];
     ...
     */
    static sun.nio.fs.NativeBuffer getNativeBuffer(int size) {
        sun.nio.fs.NativeBuffer buffer = getNativeBufferFromCache(size);
        if (buffer != null) {
    ...
    static void releaseNativeBuffer(sun.nio.fs.NativeBuffer buffer) {
        ...
        sun.nio.fs.NativeBuffer[] buffers = threadLocal.get();
        if (buffers == null) {
            buffers = new sun.nio.fs.NativeBuffer[TEMP_BUF_POOL_SIZE];
            buffers[0] = buffer;
        ...
        for (int i=0; i<TEMP_BUF_POOL_SIZE; i++) {
            sun.nio.fs.NativeBuffer existing = buffers[i];
     ...
     */
    static void copyCStringToNativeBuffer(byte[] cstr, sun.nio.fs.NativeBuffer buffer) {
     ...
     * the buffer.
     */
    static sun.nio.fs.NativeBuffer asNativeBuffer(byte[] cstr) {
        sun.nio.fs.NativeBuffer buffer = getNativeBuffer(cstr.length+1);
