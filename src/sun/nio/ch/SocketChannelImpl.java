    ...
    implements sun.nio.ch.SelChImpl
{
    ...
    SocketChannelImpl(SelectorProvider sp) throws IOException {
        super(sp);
        this.fd = sun.nio.ch.Net.socket(true);
        ...
        synchronized (stateLock) {
            ...
            if (name == StandardSocketOptions.IP_TOS) {
                if (!sun.nio.ch.Net.isIPv6Available())
                    sun.nio.ch.Net.setSocketOption(fd, StandardProtocolFamily.INET, name, value);
                return this;
            }
        ...
        throws IOException
    {
        ...
        synchronized (stateLock) {
            ...
            if (name == StandardSocketOptions.IP_TOS) {
                return (sun.nio.ch.Net.isIPv6Available()) ? (T) Integer.valueOf(0) :
                    (T) sun.nio.ch.Net.getSocketOption(fd, StandardProtocolFamily.INET, name);
            }
        ...
        synchronized (readLock) {
            ...
            try {
                ...
                for (;;) {
                    ...
                    if ((n == sun.nio.ch.IOStatus.INTERRUPTED) && isOpen()) {
                ...
                synchronized (stateLock) {
                    if ((n <= 0) && (!isInputOpen))
                        return sun.nio.ch.IOStatus.EOF;
                }
            ...
            synchronized (writeLock) {
                synchronized (stateLock) {
                    ...
                    InetSocketAddress isa = (local == null) ?
                        new InetSocketAddress(0) : sun.nio.ch.Net.checkAddress(local);
                    NetHooks.beforeTcpBind(fd, isa.getAddress(), isa.getPort());
                    sun.nio.ch.Net.bind(fd, isa.getAddress(), isa.getPort());
                    ...
                    try {
                        try {
                            ...
                            for (;;) {
                                ...
                                n = sun.nio.ch.Net.connect(fd,
                                                ia,
                                                isa.getPort());
                                if (  (n == sun.nio.ch.IOStatus.INTERRUPTED)
                                      && isOpen())
                                    continue;
                                break;
                            }
    ...
    public SocketChannel shutdownInput() throws IOException {
        synchronized (stateLock) {
            ...
            if (!isConnected())
                throw new NotYetConnectedException();
            if (isInputOpen) {
                sun.nio.ch.Net.shutdown(fd, sun.nio.ch.Net.SHUT_RD);
                if (readerThread != 0)
    ...
    public SocketChannel shutdownOutput() throws IOException {
        synchronized (stateLock) {
            ...
            if (!isConnected())
                throw new NotYetConnectedException();
            if (isOutputOpen) {
                sun.nio.ch.Net.shutdown(fd, sun.nio.ch.Net.SHUT_WR);
                if (writerThread != 0)
