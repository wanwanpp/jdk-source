    ...
    extends AsynchronousFileChannel
{
    ...
    final void ensureFileLockTableInitialized() throws IOException {
        if (fileLockTable == null) {
            synchronized (this) {
                if (fileLockTable == null) {
                    fileLockTable = sun.nio.ch.FileLockTable.newSharedFileLockTable(this, fdObj);
                }
            }
        }
    }
    ...
    final void invalidateAllLocks() throws IOException {
        if (fileLockTable != null) {
            for (FileLock fl: fileLockTable.removeAll()) {
                synchronized (fl) {
                    if (fl.isValid()) {
                        sun.nio.ch.FileLockImpl fli = (sun.nio.ch.FileLockImpl)fl;
                        implRelease(fli);
                        fli.invalidate();
                    }
                }
     ...
     */
    protected final sun.nio.ch.FileLockImpl addToFileLockTable(long position, long size, boolean shared) {
        final sun.nio.ch.FileLockImpl fli;
        try {
            ...
            fli = new sun.nio.ch.FileLockImpl(this, position, size, shared);
    ...
    protected final void removeFromFileLockTable(sun.nio.ch.FileLockImpl fli) {
     ...
     */
    protected abstract void implRelease(sun.nio.ch.FileLockImpl fli) throws IOException;
