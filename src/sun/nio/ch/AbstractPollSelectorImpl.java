    ...
    extends sun.nio.ch.SelectorImpl
{
    ...
    void putEventOps(sun.nio.ch.SelectionKeyImpl sk, int ops) {
    ...
    protected void implClose() throws IOException {
        synchronized (closeLock) {
            ...
            for(int i=channelOffset; i<totalChannels; i++) {
                sun.nio.ch.SelectionKeyImpl ski = channelArray[i];
                assert(ski.getIndex() != -1);
                ski.setIndex(-1);
                deregister(ski);
                SelectableChannel selch = channelArray[i].channel();
                if (!selch.isOpen() && !selch.isRegistered())
                    ((sun.nio.ch.SelChImpl)selch).kill();
            }
            implCloseInterrupt();
            pollWrapper.free();
            pollWrapper = null;
    ...
    protected int updateSelectedKeys() {
        ...
        for (int i=channelOffset; i<totalChannels; i++) {
            ...
            if (rOps != 0) {
                sun.nio.ch.SelectionKeyImpl sk = channelArray[i];
    ...
    protected void implRegister(sun.nio.ch.SelectionKeyImpl ski) {
        synchronized (closeLock) {
            if (closed)
                throw new ClosedSelectorException();
            ...
            if (channelArray.length == totalChannels) {
                ...
                sun.nio.ch.SelectionKeyImpl temp[] = new sun.nio.ch.SelectionKeyImpl[newSize];
                // Copy over
                for (int i=channelOffset; i<totalChannels; i++)
    ...
    protected void implDereg(sun.nio.ch.SelectionKeyImpl ski) throws IOException {
        ...
        if (i != totalChannels - 1) {
            ...
            sun.nio.ch.SelectionKeyImpl endChannel = channelArray[totalChannels-1];
