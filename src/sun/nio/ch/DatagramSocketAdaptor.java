    ...
    extends DatagramSocket
{
    ...
    private DatagramSocketAdaptor(sun.nio.ch.DatagramChannelImpl dc) throws IOException {
    ...
    public static DatagramSocket create(sun.nio.ch.DatagramChannelImpl dc) {
        try {
    ...
    {
        InetSocketAddress isa = sun.nio.ch.Net.asInetSocketAddress(remote);
        ...
        try {
            ...
            sun.nio.ch.Net.translateToSocketException(x);
        }
    }
    ...
    public InetAddress getInetAddress() {
        return (isConnected()
                ? sun.nio.ch.Net.asInetSocketAddress(dc.remoteAddress()).getAddress()
                : null);
    }

    public int getPort() {
        return (isConnected()
                ? sun.nio.ch.Net.asInetSocketAddress(dc.remoteAddress()).getPort()
                : -1);
    }
    ...
    public void send(DatagramPacket p) throws IOException {
        synchronized (dc.blockingLock()) {
            ...
            try {
                ...
                sun.nio.ch.Net.translateException(x);
            }
        }
    }
    ...
    private SocketAddress receive(ByteBuffer bb) throws IOException {
        ...
        try {
            ...
            if ((sender = dc.receive(bb)) != null)
                return sender;
            sel = sun.nio.ch.Util.getTemporarySelector(dc);
            ...
            if (sel != null)
                sun.nio.ch.Util.releaseTemporarySelector(sel);
        }
    }
