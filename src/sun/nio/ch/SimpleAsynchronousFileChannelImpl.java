    ...
    extends sun.nio.ch.AsynchronousFileChannelImpl
{
    ...
    private static class DefaultExecutorHolder {
        static final ExecutorService defaultExecutor =
            sun.nio.ch.ThreadPool.createDefault().executor();
    }

    // Used to make native read and write calls
    private static final sun.nio.ch.FileDispatcher nd = new FileDispatcherImpl();

    // Thread-safe set of IDs of native threads, for signalling
    private final sun.nio.ch.NativeThreadSet threads = new sun.nio.ch.NativeThreadSet(2);
                                  ...
                                  final CompletionHandler<FileLock,? super A> handler)
    {
        ...
        // add to lock table
        final sun.nio.ch.FileLockImpl fli = addToFileLockTable(position, size, shared);
        if (fli == null) {
            Throwable exc = new ClosedChannelException();
            if (handler == null)
                return sun.nio.ch.CompletedFuture.withFailure(exc);
            sun.nio.ch.Invoker.invokeIndirectly(handler, attachment, null, exc, executor);
            return null;
        }

        final sun.nio.ch.PendingFuture<FileLock,A> result = (handler == null) ?
            new sun.nio.ch.PendingFuture<FileLock,A>(this) : null;
            ...
            public void run() {
                ...
                try {
                    ...
                    try {
                        ...
                        if (n != sun.nio.ch.FileDispatcher.LOCKED || !isOpen()) {
                ...
                if (handler == null) {
                    ...
                    sun.nio.ch.Invoker.invokeUnchecked(handler, attachment, fli, exc);
                }
            }
        };
        ...
        throws IOException
    {
        ...
        // add to lock table
        sun.nio.ch.FileLockImpl fli = addToFileLockTable(position, size, shared);
        if (fli == null)
        ...
        try {
            ...
            if (n == sun.nio.ch.FileDispatcher.NO_LOCK)
                return null;    // locked by someone else
            if (n == sun.nio.ch.FileDispatcher.INTERRUPTED)
                throw new AsynchronousCloseException();
            // should not get here
                                 ...
                                 final CompletionHandler<Integer,? super A> handler)
    {
        ...
        if (!isOpen() || (dst.remaining() == 0)) {
            ...
            if (handler == null)
                return sun.nio.ch.CompletedFuture.withResult(0, exc);
            sun.nio.ch.Invoker.invokeIndirectly(handler, attachment, 0, exc, executor);
            return null;
        }

        final sun.nio.ch.PendingFuture<Integer,A> result = (handler == null) ?
            new sun.nio.ch.PendingFuture<Integer,A>(this) : null;
        Runnable task = new Runnable() {
            public void run() {
                int n = 0;
                Throwable exc = null;
                ...
                try {
                    begin();
                    do {
                        n = sun.nio.ch.IOUtil.read(fdObj, dst, position, nd, null);
                    } while ((n == sun.nio.ch.IOStatus.INTERRUPTED) && isOpen());
                    if (n < 0 && !isOpen())
                        throw new AsynchronousCloseException();
                } catch (IOException x) {
                ...
                if (handler == null) {
                    ...
                    sun.nio.ch.Invoker.invokeUnchecked(handler, attachment, n, exc);
                }
            }
        };
        executor.execute(task);
        ...
        Runnable task = new Runnable() {
            public void run() {
                ...
                try {
                    begin();
                    do {
                        n = sun.nio.ch.IOUtil.write(fdObj, src, position, nd, null);
                    } while ((n == sun.nio.ch.IOStatus.INTERRUPTED) && isOpen());
                    if (n < 0 && !isOpen())
                        throw new AsynchronousCloseException();
                } catch (IOException x) {
