    ...
    extends AsynchronousChannelGroup implements Executor
{
                                 ...
                                 sun.nio.ch.ThreadPool pool)
    {
        ...
        this.timeoutExecutor = (ScheduledThreadPoolExecutor)
            Executors.newScheduledThreadPool(1, sun.nio.ch.ThreadPool.defaultThreadFactory());
    ...
    private Runnable bindToGroup(final Runnable task) {
        ...
        return new Runnable() {
            public void run() {
                sun.nio.ch.Invoker.bindToGroup(thisGroup);
                task.run();
            }
        };
    }
    ...
    private void startInternalThread(final Runnable task) {
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            ...
            public Void run() {
                ...
                sun.nio.ch.ThreadPool.defaultThreadFactory().newThread(task).start();
                return null;
            }
         });
    }
    ...
    final int threadExit(Runnable task, boolean replaceMe) {
        if (replaceMe) {
            try {
                if (sun.nio.ch.Invoker.isBoundToAnyGroup()) {
