...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    private static boolean isRunning(int c) {
        return c < SHUTDOWN;  //状态值比shutdown小的只有running。
    }
    ...
    private boolean compareAndIncrementWorkerCount(int expect) {
        return ctl.compareAndSet(expect, expect + 1);   //clt表示当前工作线程数
    }
...
//    空闲时间
        ...
        try {
            ...
            if (t != null) {
                ...
                try {
                    ...
                    int c = ctl.get();
                    int rs = runStateOf(c);//表示运行状态

                    //shutdown状态    不接受新任务，但是仍然执行队列中的任务
                    if (rs < SHUTDOWN ||//只有running状态的值小于shutdown.
            ...
            if (!workerStarted)//启动失败则回滚，如workers.remove(w);
                addWorkerFailed(w);
        }
    ...
    public void execute(Runnable command) {
        ...
        if (isRunning(c) && workQueue.offer(command)) {//工作线程数量不小于核心线程，且线程池在运行状态下，则将任务放入队列中。
