    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            if (!readerShouldBlock() &&
                ...
                //设置firstReader，firstReaderHoldCount，cachedHoldCounter，readHolds等方便记录线程获取读锁的次数
                if (r == 0) {
                    firstReader = current;
                    ...
                    HoldCounter rh = cachedHoldCounter;
                    //如果还没有cachedHoldCounter，或者cachedHoldCounter锁代表的线程不是当前线程
                    if (rh == null || rh.tid != current.getId())
        ...
        final int fullTryAcquireShared(Thread current) {
            ...
            for (; ; ) {
                int c = getState();
                //其他线程获取了写锁 ，返回失败             若是当前线程获取了写锁或者写锁未被任何线程获取，则可以正常添加读锁同步状态
                if (exclusiveCount(c) != 0) {
                ...
                if (sharedCount(c) == MAX_COUNT)
                    throw new Error("Maximum lock count exceeded");
                if (compareAndSetState(c, c + SHARED_UNIT)) {//获取成功
                    if (sharedCount(c) == 0) {
                        firstReader = current;
                        firstReaderHoldCount = 1;
                    } else if (firstReader == current) {
