...
//    要求Map中的键实现Comparable接口，进行比较时会使用到Comparable接口中的compareTo方法
    public TreeMap() {
        comparator = null;
    }
...
//    会使用SortedMap中的comparator
    ...
    //若comparator不为空，则使用comparator进行比较，否则使用comparable.compareTo方法
     ...
     * 大致过程为：
     * 1.判断有无根节点
     * 2.若有根节点则通过comparator或者comparable寻找合适的父节点parent
     * 3.确定是作为左孩子还是右孩子并建立关系
     * 4.调整树的结构，维持红黑树平衡约束
     */
    public V put(K key, V value) {
        ...
        if (t == null) {
            ...
            //这里的目的不是为了比较，而是为了检查key的类型和null，如果类型不匹配或为null，compare方法会抛出异常。
    ...
    private void deleteEntry(Entry<K, V> p) {
        ...
        // point to successor.
        //这里处理的就是两个孩子的情况，s为后继，当前节点p的key和value设置
        // 为了s的key和value，然后将待删节点p指向了s，这样就转换为了一个孩子或叶子节点的情况。
        if (p.left != null && p.right != null) {
            Entry<K, V> s = successor(p);//s为后继节点
            p.key = s.key;
            p.value = s.value;
            p = s;
        } // p has 2 children
